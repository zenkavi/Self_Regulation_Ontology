...
command_head = 'python /corral-repl/utexas/poldracklab/users/zenkavi/Self_Regulation_Ontology/data_preparation/download_retest_data_tacc_newapi.py "Self Regulation Retest Battery"'

command_start_page = '"http://www.expfactory.org/new_api/results/62/?page' + start_page + '"'
if start_page == '1':
    command_start_page = '"http://www.expfactory.org/new_api/results/62/"'
command_end_page = '"http://www.expfactory.org/new_api/results/62/?page=' + str(int(start_page)+int(split_by)-1)+ '"'
...
for j in range(int(num_commands)/int(num_commands_in_file)+1):
    f = open('./launch_scripts_newapi/exec_download_retest_data_tacc_'+str(j+1)+'.sh', 'w')
    f.write('#!/bin/bash' + "\n")
