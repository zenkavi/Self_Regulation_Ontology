...
# drop variables with high correlations to other variables 
# within same task
corrvars=c('probabilistic_selection.value_sensitivity','angling_risk_task_always_sunny.release_score','angling_risk_task_always_sunny.keep_score')
for (c in corrvars){
  taskdata_test[c]=NULL
}
print(sprintf('dropping %d variables with high correlations',length(corrvars)))

# drop variables with highly skewed distributions

# compute skewness for each variable

skw=array(dim=dim(taskdata_test)[2])
skewvars=c()
for (i in 1:dim(taskdata_test)[2]){
  n=names(taskdata_test)[i]
  skw[i]=skewness(as.matrix(taskdata[n]))
  if (abs(skw[i])  > 1){
    print(sprintf('%f:%s',skw[i],n))
    skewvars=c(skewvars,n)
  }
}
...
print(sprintf('dropping %d variables with excessive skew',length(skewvars)))
